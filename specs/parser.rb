...
describe 'Log parser' do
  ...
  it 'ignores netsplits' do
    m = Logformat::Message.parse_irssi_line('2016-10-15', '#somechannel', '04:06 -!- Netsplit irc1.example.com <-> irc2.example.com quits: sonja')
    expect(m).to be_nil
    expect(Logformat::Message.count).to eql 0
  end
  ...
  it 'ignores messages with double time' do
    m = Logformat::Message.parse_irssi_line('2016-10-15', '#somechannel', '14:16 12:17 < udo> jürgen, die logs')
    expect(m).to be_nil
    expect(Logformat::Message.count).to eql 0
  end
  ...
  it 'ignores lines with zero characters' do
    m = Logformat::Message.parse_irssi_line('2016-01-15', '#somechannel', "\u0000\u0000\u0000")
    expect(m).to be_nil
    expect(Logformat::Message.count).to eql 0
  end
